...
```
> 上例中 `invert_category_table()` 對 `2330` 反查結果為 `["IC 製造", "先進封裝"]`（依分類 key 在檔案內的出現順序），最終顯示為 `[大型, 半導體, IC 製造, 先進封裝]`。
...
```
2330 台積電 [大型, 半導體]:賣超 35.3 億元 (量能, 大額，外 +5,795 張 / 投 +184 張 / 自 +457 張)
```

- `[]` 內為**分類標籤**：市值分級（既有）＋產業別（新增，FinMind 原始值去尾字「業」，已確認）＋概念標籤（新增，如有）。
