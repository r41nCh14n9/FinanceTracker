...
| 第十一輪 | Roy Chiang 要求提供統一投信頁面 URL 自行檢查，發現「基金明細頁」（`/ETF/Fund/Info`）上也顯示「資料日期」，追問能否比照富邦直接用這頁判斷新鮮度、不用下載 Excel。深入檢查該頁原始碼（Vue.js SPA）後發現：頁面上顯示的「資料日期」是前端計算屬性、非可靠來源，但其計算依據（`assetDetails`）最終指向一段**內嵌在頁面 HTML 裡的完整結構化 JSON**（`id="DataAsset"` 的 `data-content` 屬性），依 `AssetCode` 分組，`AssetCode == "ST"` 即成分股清單，每筆明細帶 `TranDate`（可驗證新鮮度）與 `EditTime`（實際更新時間戳記，可供 §六待確認事項作業窗口佐證）。已整支重寫 `uni.py`：改解析這段內嵌 JSON，移除 Excel 匯出下載／`openpyxl` 解析邏輯及套件相依。**另外實測 5 種常見日期查詢參數命名（`date`／`qDate`／`tranDate`／`ddate`／`assetDate`）皆對這頁的資料內容無效**，確認統一分類維持 A（無日期查詢能力），本輪只是簡化技術實作、未解鎖回補能力，`SUPPORTS_BACKFILL` 維持 `False`；同步更新分類表統一列說明、新增統一資料來源簡化更正記錄、§四內部元件設計新增 `uni.py` 項目 |
...
| 分類 | 投信 | 說明 |
| :--- | :--- | :--- |
| **A. 持倉明細 API 完全沒有日期參數可帶**（官網介面設計本身只回傳當下最新一期，不是程式漏寫） | 統一（1 家） | 回應內容附一個日期欄位可事後比對，本質上是「碰運氣」而非「查詢」；2026-08-24 已實測過 `date`／`qDate`／`tranDate`／`ddate`／`assetDate` 等 5 種常見命名的查詢參數，皆無效果，確認真的沒有日期查詢能力，非漏查 |
...
**統一投信資料來源簡化（2026-08-24，分類不變，維持 A）：** Roy Chiang 發現統一「基金明細頁」（`/ETF/Fund/Info`）其實把完整的資產組合以結構化 JSON 內嵌在頁面裡（`id="DataAsset"` 的 `data-content` 屬性），每筆持股明細帶 `TranDate`（交易日期）與 `EditTime`（實際更新時間戳記），資料品質遠優於原本抓取的 Excel 匯出檔（`AssetExcelNPOI`），且不需要 `openpyxl` 處理 NPOI 相容性問題。已將 `uni.py` 整支改為解析這個內嵌 JSON，移除 Excel 下載/解析邏輯與 `openpyxl` 相依套件（`requirements.txt` 同步移除）。**已實測過這頁的 5 種常見日期查詢參數命名（`date`／`qDate`／`tranDate`／`ddate`／`assetDate`）皆無效果，證實這頁本身沒有日期查詢能力**，跟原本 Excel 端點的限制相同，分類維持 A（僅解決「用什麼技術手段取值」，不解決「能否查歷史日期」），`SUPPORTS_BACKFILL` 仍為 `False`。
...
| `src/issuer_pcf/uni.py`（`UniPcfAdapter`） | **已完成（2026-08-24）：** 整支改為呼叫「基金明細頁」（`/ETF/Fund/Info`）並解析內嵌的 `id="DataAsset"` `data-content` JSON（`AssetCode == "ST"` 群組），比對明細 `TranDate` 與 `snapshot_date`，不符則回傳空清單；**移除**原本的 Excel 匯出下載/`openpyxl` 解析邏輯。已實測 5 種常見日期查詢參數命名皆無效，確認頁面本身無日期查詢能力，`SUPPORTS_BACKFILL` 維持 `False`（分類不變，僅簡化技術手段），已用真實網站驗證並跑過測試 | 🔴 重寫（連帶 `requirements.txt` 移除 `openpyxl` 相依套件） |
