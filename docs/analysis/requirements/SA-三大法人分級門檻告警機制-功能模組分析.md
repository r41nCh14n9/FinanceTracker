...
| 編號 | 功能 | 說明 |
| :--- | :--- | :--- |
| FR-3.3（🟡 修改） | 差異化告警訊息格式化 | 個股每檔一行，於 `[市值分級, 觸發原因...]` 標籤內標示觸發原因（量能／大額，兩者皆觸發則並列）；新增「◆ 大盤三大法人動態」獨立區塊，僅列出當日**有觸發門檻**的法人類別（例如僅外資達標時，只顯示外資該行，不強制列出三行）；訊息不附加免責文字頁尾 |
...
◆ 三大法人買賣超（個股）
  2330 台積電 [大型, 大額]:賣超 57.3 億元 (外 -5,731 張 / 投 +85 張 / 自 -178 張)
  2454 聯發科 [大型, 量能]:賣超 12.1 億元 (外 -1,850 張 / 投 +120 張 / 自 -95 張)
```
...
| 觸發原因可追溯 | 報告內容可區分「量能」與「大額」（標籤形式，兩者皆觸發時並列）；金額估算方式僅於文件層級註明，訊息內不重複標示（見 SD §六第 7 項決策） | 人工於實機 LINE 檢視 |
