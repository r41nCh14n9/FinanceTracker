...
上方「端到端驗證紀錄」的簡報範例為 2026-08-04 實測當下的真實輸出，**予以保留、不回頭改寫**；但 `MessageFormatter` 的訊息格式其後已再次調整（個股標籤改為 `[市值分級, 觸發原因...]` 單行呈現、移除頁尾免責文字），現行格式以 [notifier.py](../../../src/notifier.py) 與最新版 [SA-三大法人分級門檻告警機制-功能模組分析.md](../../analysis/requirements/SA-三大法人分級門檻告警機制-功能模組分析.md)／[SD-三大法人買賣超關注清單通知-系統設計書.md](../../design/architecture/SD-三大法人買賣超關注清單通知-系統設計書.md) 為準。
