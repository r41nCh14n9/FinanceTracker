...
- 全量單元測試：**172 個測試全數通過**（本輪新增 3 個 Kgi 測試）

## 待辦與已知限制

- [ ] `009816` 目前是唯一登記的凱基投信 ETF，內部代碼對照為手動維護的靜態表；若凱基投信旗下有其他 ETF 需要納入監控，需另外查出對應的 `fundID` 並加進 `_FUND_ID_BY_TICKER`，或後續找到動態查詢端點後改用比照國泰/群益模式的自動解析
